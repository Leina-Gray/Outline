        ...
        if style_data and isinstance(style_data, dict):
            ...
            for prop, val in style_data.items():
                if isinstance(val, dict):
                    # Smart "Reach Inside" Logic: 
                    # If the value is a dict, it's a nested selector (e.g., h1: { color: red })
                    nested_rule = f".{el_id} {prop} {{\n"
                    for n_prop, n_val in val.items():
                        nested_rule += f"  {n_prop.replace(' ', '-')}: {n_val};\n"
                    nested_rule += "}"
                    css_rules_list.append(nested_rule)
                else:
                    # Normal direct style
                    main_styles.append(f"  {prop.replace(' ', '-')}: {val};")
...
def compile_outline():
    try:
        ...
        output = template_str.replace("{{ CSS }}", "\n".join(css_rules)).replace("{{ CONTENT }}", body_content)
        with open("index.html", "w") as f: f.write(output)
        print("Done! Smart styles applied.")
    except Exception as e:
        print(f"Error: {e}")
